class PullRequestsController < AuthenticatedController
  def index
    ...
    closed_pull_requests_ids = pull_requests.reject(&:open?).map(&:github_id)
  ...
  def create
    ...
    SlackNotifier.post_message(channel: current_team.slack_channel, pull_request: pull_request, pull_request_blurb: blurb)
    render json: ::PullRequestSerializer.as_json(pull_request), github_user: github_user
  end

  def bump
    blurb =
      PullRequestBlurb.where("github_repo = ? and github_id = ?", pull_request_blurb_params[:github_repo], pull_request_blurb_params[:github_id])
        .first
    ...
    SlackNotifier.post_message(channel: current_team.slack_channel, pull_request: pull_request, pull_request_blurb: blurb, bump: true) if pull_request
